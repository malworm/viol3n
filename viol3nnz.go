...
func TemplateTextReplace(ParamOne string, ParamTwo string, ParamThree string, ParamFour string, ParamFive string) {
	input, err := ioutil.ReadFile("Template.go")
	...
	output = bytes.Replace(output, []byte("FileNameVioL3NNz"), []byte(string(ParamThree)), -1)
...
func ShowMenu() {
	ClearScreen()
	fmt.Println(CYAN, "                                         Author: Viol3nnZ")
